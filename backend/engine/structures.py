...
This module deliberately imports only SymPy (and `engine.solver`, which also
only imports SymPy) so the host verify script can load it without the Google
...
# Coefficient pools for slot templates. {v} is replaced with the variable,
# {n} with an exponent. All values are positive — sign variety comes from the
# template structure.
_COEFF_POOLS = {
    "a": ["3", "5", "2", "4", "7", "6"],
    "b": ["2", "4", "6", "3", "8", "1", "5"],
...
def build_sample(struct, seed):
    ...
    for attempt in range(40):
        ...
        try:
            solution = solve("integral", struct["question_type"], params)
        except Exception:
            continue

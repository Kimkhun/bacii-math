"""Topic-neutral kernel: question-type registry, complex-number string
formatting, and the small SymPy utilities every ``engine.topics.*`` solver
module uses.

Imports only SymPy so the topic structure-registry modules (and the host
verify scripts) can load this without the Google LLM client stack.
"""

...
arithmetic/power/De Moivre/nth-root pools), the Gemini-proposed variant
(SymPy still validates), and the curated-textbook loader (below)."""
import glob
import json
import os
import random
...
from engine.core.shared import QUESTION_TYPES, format_z, z_latex
from engine.notation import pretty_expr

from .nth_roots import _solve_nth_root
from .trig import STANDARD_ANGLES, angle_latex, z_from_polar
...
def _build_curated_complex(item, difficulty):
    """A real textbook exercise from data/curated/{formula_name}.json, replayed
    through the same a+bi solver as the procedural pool (SymPy still
    computes/grades the answer; the exam-authored technique note is carried
    for reference, mirroring the limit topic's curated pool)."""
...
def _build_arithmetic(a1, b1, a2, b2, operation, difficulty):
    ...
    return {
        ...
        "z_display": f"z1 = {format_z(a1, b1)}, z2 = {format_z(a2, b2)}",
...
# part — sorted by technique into data/curated/{formula_name}.json. Most
# textbook exercises involve powers, radicals, or systems the current solver
# (plain a+bi in, one of 5 question types out) can't replay, so only the
# literal-integer subset is parsed here; everything else is skipped, same as
# the limit topic's curated loader's graceful-skip pattern.
# ---------------------------------------------------------------------------
...
def _classify_question_type(prompt_latex):
    low = prompt_latex.lower()
    for keyword, qt in _QUESTION_TYPE_KEYWORDS:
        if keyword in low:
            return qt
    return None
...
def _load_complex_curated():
    items = []
    for fpath in sorted(glob.glob(os.path.join(_CATALOG_DIR, "*.json"))):
        try:
        ...
        for ex in data.get("exercises", []):
            ...
            if intro_idx == -1:
                continue
            m = _LITERAL_RE.match(prompt_latex, intro_idx)
            if not m:
                continue
            qt = _classify_question_type(prompt_latex)
            if qt is None:
                continue
            a = int(m.group(1))
